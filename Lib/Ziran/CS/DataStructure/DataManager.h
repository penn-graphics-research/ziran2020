...
public:
    ...
    template <class Type>
    DataArray<Type>* getPointer(const AttributeName<Type>& a) const
    {
        auto search = batches.find(a.hash);
        return search != batches.end() ? &dynamic_cast<DataArray<Type>&>(*search->second) : nullptr;
    }
    ...
    */
    ...
    void remove(int old_index)
    {
        --count;
        for (const auto& b : batches) {
            b.second->remove(old_index);
        }
    }
};
