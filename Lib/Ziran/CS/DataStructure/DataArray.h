...
public:
    ...
    virtual void remove(int old_index) override
    {
        int id_in_array = valueId(old_index);
        if (id_in_array != -1) {
            array.erase(array.begin() + id_in_array);
        }
        for (int i = 0; i < (int)ranges.ranges.size(); ++i) {
            int lower = ranges.ranges[i].lower;
            int upper = ranges.ranges[i].upper;
            if (lower <= old_index && old_index < upper) {
                if (lower + 1 == upper) {
                    ranges.ranges.erase(ranges.ranges.begin() + i);
                }
                else {
                    --ranges.ranges[i].upper;
                }
                break;
            }
        }
        for (int i = 0; i < (int)ranges.ranges.size(); ++i)
            if (old_index < ranges.ranges[i].lower) {
                --ranges.ranges[i].lower;
                --ranges.ranges[i].upper;
            }
    }
