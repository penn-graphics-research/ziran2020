...
#include <Ziran/Sim/DeformableObjectHandleCore.h>
...
public:
    ...
    template <class TMesh>
    DeformableObjectHandle<T, dim, TMesh> addDeformableObject(MeshHandle<T, dim, TMesh>& handle)
    {
        using TElementManager = ElementManagerFor<T, TMesh, dim>;
        ZIRAN_INFO(" Adding mesh with ", handle.mesh->numberElements(), " elements and ", handle.particle_range.length(), " particles");
        TElementManager& elements = scene.getElementManager();
        addMeshToWrite(*handle.mesh, handle.particle_index_offset);

        return DeformableObjectHandle<T, dim, TMesh>(handle, elements, scene.forces);
    };

    template <class TMesh>
    DeformableObjectHandle<T, dim, TMesh> addDeformableObjectNoWrite(MeshHandle<T, dim, TMesh>& handle)
    {
        using TElementManager = ElementManagerFor<T, TMesh, dim>;
        ZIRAN_INFO(" Adding non-writing mesh with ", handle.mesh->numberElements(), " elements and ", handle.particle_range.length(), " particles");
        TElementManager& elements = scene.getElementManager();
