    ...
    int solve(const TM& A, TV& x, const TV& b, const bool verbose = false)
    {
        ...
        for (cnt = 0; cnt < Base::max_iterations; ++cnt) {
            if (residual_preconditioned_norm <= local_tolerance) {
                ZIRAN_INFO("\tCG terminates at ", cnt, "; (preconditioned norm) residual = ", residual_preconditioned_norm);
                return cnt;
            }
        ...
        ZIRAN_INFO("ConjugateGradient max iterations reached ", Base::max_iterations);
        return Base::max_iterations;
    }
};
} // namespace ZIRAN
