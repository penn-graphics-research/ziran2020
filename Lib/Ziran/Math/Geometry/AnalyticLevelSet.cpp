...
template <class T, int dim>
IntersectionLevelSet<T, dim>::IntersectionLevelSet(StdVector<std::unique_ptr<AnalyticLevelSet<T, dim>>>&& level_sets)
    : level_sets(std::move(level_sets))
{
}

template <class T, int dim>
IntersectionLevelSet<T, dim>::IntersectionLevelSet(const StdVector<std::unique_ptr<AnalyticLevelSet<T, dim>>>& level_sets_input)
{
    for (const auto& ls : level_sets_input)
        level_sets.emplace_back(ls->createPtr());
}
...
template <class T, int dim>
void IntersectionLevelSet<T, dim>::add(const AnalyticLevelSet<T, dim>& ls1, const AnalyticLevelSet<T, dim>& ls2)
{
    level_sets.emplace_back(ls1.createPtr());
    level_sets.emplace_back(ls2.createPtr());
}
...
template <class T, int dim>
T IntersectionLevelSet<T, dim>::signedDistance(const TV& X) const
{
    auto& setA = level_sets[0];
    auto& setB = level_sets[1];
    return std::max(setA->signedDistance(X), (setB->signedDistance(X)));
...
template <class T, int dim>
typename IntersectionLevelSet<T, dim>::TV IntersectionLevelSet<T, dim>::normal(const TV& X) const
{
    auto& setA = level_sets[0];
    auto& setB = level_sets[1];
    if (setB->signedDistance(X) > setA->signedDistance(X))
...
template <class T, int dim>
void IntersectionLevelSet<T, dim>::getBounds(TV& min_corner, TV& max_corner) const
{
    level_sets[0]->getBounds(min_corner, max_corner);
}

template <class T, int dim>
T IntersectionLevelSet<T, dim>::volume() const
{
    return level_sets[0]->volume() - level_sets[1]->volume(); //TODO not sure about this one
}

template <class T, int dim>
std::unique_ptr<AnalyticLevelSet<T, dim>> IntersectionLevelSet<T, dim>::createPtr() const
{
    return std::make_unique<IntersectionLevelSet<T, dim>>(level_sets);
}
...
template class IntersectionLevelSet<double, 2>;
template class IntersectionLevelSet<double, 3>;
template class IntersectionLevelSet<float, 2>;
template class IntersectionLevelSet<float, 3>;
