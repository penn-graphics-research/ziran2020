...
template <class T, int dim>
bool AnalyticCollisionObject<T, dim>::detectAndResolveCollisionWithObjectSpeed(const TV& x, TV& v, TV& n, TV& v_object) const
{
    if (type == GHOST)
        return false;
    ...
    */
    ...
    if (colliding) {
        v_object = omega.cross(x_minus_b) + (dsdt * one_over_s) * x_minus_b + R.rotation * s * getMaterialVelocity(X) + dbdt;
        v -= v_object;
        if (type == STICKY) {
            v.setZero();
        }
