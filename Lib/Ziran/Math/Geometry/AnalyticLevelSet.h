...
*/
template <class T, int dim>
class IntersectionLevelSet : public AnalyticLevelSet<T, dim> {
public:
    using TV = Vector<T, dim>;
    using TM = Matrix<T, dim, dim>;
    StdVector<std::unique_ptr<AnalyticLevelSet<T, dim>>> level_sets;

    IntersectionLevelSet() {}

    IntersectionLevelSet(StdVector<std::unique_ptr<AnalyticLevelSet<T, dim>>>&& level_sets);

    IntersectionLevelSet(const StdVector<std::unique_ptr<AnalyticLevelSet<T, dim>>>& level_sets_input);
...
public:
    ...
    /**
       ...
       The input transformation (rotation R, translation b)
       applies to the primitive as R*c+b
       Note that q_in forms quaternion <w, x, y, z> = <q(0), q(1), q(2), q(3)>
     */
