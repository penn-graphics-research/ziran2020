...
void SimplexMesh<dim>::initializeBoundaryElements()
{
    ...
    HashTable<int, bool> node_hash;
    for (auto face : boundary_indices)
        for (int i = 0; i < boundary_vertices; i++)
            node_hash.insert(face(i), true);
    boundary_nodes.clear();
    for (auto it = node_hash.cbegin(); it != node_hash.cend(); it++)
        boundary_nodes.emplace_back(std::move(it->key));
}
