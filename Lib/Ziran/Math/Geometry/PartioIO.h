...
void readPartio(const std::string& particleFile, StdVector<Vector<T, dim>>& x)
{
    ZIRAN_INFO("Reading ", particleFile);
    x.clear();
...
template <class T, int dim>
void writeSandPartio(const std::string& particleFile,
    const Particles<T, dim>& particles)
{
    // TODO: put assert that you have either StvkWithHenckyIsotropic or any of its variants and DruckerPragerWithHencky
    AttributeName<T> mass = Particles<T, dim>::mass_name();
    AttributeName<Vector<T, dim>> x_name = Particles<T, dim>::X_name();
    AttributeName<Vector<T, dim>> v_name = Particles<T, dim>::V_name();
    AttributeName<Eigen::Matrix<T, dim, dim>> f_name = F_name<T, dim>();
    auto dpsh_plasticity_name = DruckerPragerStrainSoftening<T>::attributeName();
    AttributeName<T> sand_saturation_name = saturation_name<T>();
    auto stvkhi_name = AttributeName<StvkWithHenckyIsotropic<T, dim>>(StvkWithHenckyIsotropic<T, dim>::name());

    Partio::ParticlesDataMutable* parts = Partio::create();
    Partio::ParticleAttribute posH, vH, mH, f_row_0H, f_row_1H, f_row_2H, detH, dpsh_plasticityH, sand_satH, youngs_modH;
    mH = parts->addAttribute("m", Partio::VECTOR, 1);
    posH = parts->addAttribute("position", Partio::VECTOR, 3);
    vH = parts->addAttribute("v", Partio::VECTOR, 3);
    f_row_0H = parts->addAttribute("f_row_0", Partio::VECTOR, 3);
    f_row_1H = parts->addAttribute("f_row_1", Partio::VECTOR, 3);
    f_row_2H = parts->addAttribute("f_row_2", Partio::VECTOR, 3);
    detH = parts->addAttribute("det", Partio::VECTOR, 1);
    dpsh_plasticityH = parts->addAttribute("coh", Partio::VECTOR, 1);
    sand_satH = parts->addAttribute("sand_sat", Partio::VECTOR, 1);
    youngs_modH = parts->addAttribute("youngs_mod", Partio::VECTOR, 1);

    auto iter = particles.iter(mass, x_name, v_name, f_name, dpsh_plasticity_name, sand_saturation_name, stvkhi_name);
    for (; iter; ++iter) {
        int idx = parts->addParticle();
        float* m = parts->dataWrite<float>(mH, idx);
        float* p = parts->dataWrite<float>(posH, idx);
        float* v = parts->dataWrite<float>(vH, idx);
        float* f_row_0 = parts->dataWrite<float>(f_row_0H, idx);
        float* f_row_1 = parts->dataWrite<float>(f_row_1H, idx);
        float* f_row_2 = parts->dataWrite<float>(f_row_2H, idx);
        float* det = parts->dataWrite<float>(detH, idx);
        float* coh = parts->dataWrite<float>(dpsh_plasticityH, idx);
        float* sat = parts->dataWrite<float>(sand_satH, idx);
        float* youngs_mod = parts->dataWrite<float>(youngs_modH, idx);
        ...
        Vector<T, dim> X = iter.template get<1>();
        Vector<T, dim> V = iter.template get<2>();
        Eigen::Matrix<T, dim, dim> F = iter.template get<3>();
        auto& dpsh = iter.template get<4>();
        T sand_sat = iter.template get<5>();
        auto& cm_elastic = iter.template get<6>();
        ...
        T lambda = cm_elastic.lambda;
        T mu = cm_elastic.mu;
        youngs_mod[0] = mu * (3 * lambda + 2 * mu) / (lambda + mu);
    }
...
template <class T, int dim>
void writeSandPartioUnilateral(const std::string& particleFile,
    const Particles<T, dim>& particles)
{
    // TODO: put assert that you have either StvkWithHenckyIsotropic or any of its variants and DruckerPragerWithHencky
    AttributeName<T> mass = Particles<T, dim>::mass_name();
    ...
    AttributeName<Vector<T, dim>> v_name = Particles<T, dim>::V_name();
    AttributeName<Eigen::Matrix<T, dim, dim>> f_name = F_name<T, dim>();
    auto dpsh_plasticity_name = DruckerPragerStvkHencky<T>::attributeName();
    AttributeName<T> sand_saturation_name = saturation_name<T>();
    auto stvkhiu_name = StvkWithHenckyIsotropicUnilateral<T, dim>::attributeName();
    ...
    auto iter = particles.iter(mass, x_name, v_name, f_name, dpsh_plasticity_name, sand_saturation_name, stvkhiu_name);
    for (; iter; ++iter) {
        int idx = parts->addParticle();
        float* m = parts->dataWrite<float>(mH, idx);
        float* p = parts->dataWrite<float>(posH, idx);
