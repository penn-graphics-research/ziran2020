...
#else
    pd.setDistanceByParticlesPerCell(grid_dx, particles_per_cell * 2);
    StdVector<TV> unscaled_samples;
    if (dim == 3)
        pd.sampleFromPeriodicData(unscaled_samples, [&](const TV& x) { return bb.inside(x); });
    else
        pd.sample(unscaled_samples, [&](const TV& x) { return bb.inside(x); });
    T real_ppc = (T)unscaled_samples.size() / ((max_corner - min_corner) / grid_dx).prod();
