...
template <class T, int _dim>
T NeoHookeanBorden<T, _dim>::getSigmaCrit(T percentage) const
{
    Scratch s;
...
//
// Get effective Cauchy stress: non-degraded stress
//
template <class T, int _dim>
void NeoHookeanBorden<T, _dim>::evaluateEffectiveCauchyStress(const TM& F, TM& cauchystress)
{ // let a dummy model copied from the real model call this function
    g = 1; // undegrade.
    Scratch scratch_dummy;
    updateScratch(F, scratch_dummy);
    TM kirchhoff_stress;
    kirchhoff(scratch_dummy, kirchhoff_stress);
    cauchystress = kirchhoff_stress / scratch_dummy.J;
}
