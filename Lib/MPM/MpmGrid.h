...
template <class T, int dim, int degree = ZIRAN_MPM_DEGREE>
...
public:
    ...
    BSplineWeightsWithDDW(const Vector<T, dim>& X, T dx)
        : dx(dx), one_over_dx(1 / dx), base_node(Vector<int, dim>::Zero())
    {
        computeWithDDW(X);
    }

    void computeWithDDW(const Vector<T, dim>& X)
    {
        Vector<T, dim> X_index_space = one_over_dx * X;
        for (int d = 0; d < dim; d++)
...
public:
    ...
    bool activate_empty_cell = false;
    ...
    void pollute(const StdVector<Vector<T, dim>>& Xarray, T dx, int start = 0, int end = 1, const Vector<T, dim>& anchor = Vector<T, dim>::Zero())
    {
        StdVector<uint64_t> particle_base_offset(Xarray.size());
        page_map->Clear();
        for (int i = 0; i < (int)Xarray.size(); ++i) {
            BSplineWeights<T, dim, degree> spline(Xarray[i] + anchor, dx);
            uint64_t offset = SparseMask::Linear_Offset(to_std_array(spline.base_node));
            particle_base_offset[i] = offset;
            if (!i || (particle_base_offset[i] != particle_base_offset[i - 1])) {
                ...
                if constexpr (dim == 2) {
                    ...
                    for (int i = start; i <= end; ++i)
                        for (int j = start; j <= end; ++j)
                            page_map->Set_Page(SparseMask::Packed_Add(
        ...
        for (int b = 0; b < (int)blocks.second; ++b) {
            ...
            std::memset(&grid_array(base_offset), 0, (size_t)(1 << MpmGrid<T, dim, degree>::log2_page));
            auto* g = reinterpret_cast<GridState<T, dim>*>(&grid_array(base_offset));
    ...
    int getNumNodes()
    {
        ...
        for (int b = 0; b < (int)blocks.second; ++b) {
            ...
            for (int i = 0; i < (int)SparseMask::elements_per_block; ++i)
                if (g[i].m != 0 || activate_empty_cell)
    ...
    template <typename OP>
    inline void iterateKernelWithValidation(const BSplineWeights<T, dim, degree>& spline, uint64_t base_offset, const OP& target)
    {
        auto grid_array = grid->Get_Array();
        ...
        if constexpr (dim == 2) {
            for (int i = 0; i < interpolation_degree + 1; ++i) {
                ...
                for (int j = 0; j < interpolation_degree + 1; ++j) {
                    ...
                    auto offset = SparseMask::Packed_Add(base_offset, SparseMask::Linear_Offset(i, j));
                    if (!page_map->Test_Page(offset)) continue;
                    GridState<T, dim>& g = reinterpret_cast<GridState<T, dim>&>(grid_array(offset));
    ...
    template <typename OP>
    static inline void iterateArena(const BSplineWeights<T, dim, degree>& spline, const OP& target)
    {
        T one_over_dx = spline.one_over_dx;
        auto& w = spline.w;
        auto& dw = spline.dw;
        ...
        if constexpr (dim == 2) {
            for (int i = 0; i < interpolation_degree + 1; ++i) {
                ...
                for (int j = 0; j < interpolation_degree + 1; ++j) {
                    ...
                    T dwijdxj = wi * one_over_dx * dw[1](j);
                    coord[1] = base_coord[1] + j;
                    target(coord, wij, Vector<T, dim>{ dwijdxi, dwijdxj });
                }
            }
        }
        else {
            for (int i = 0; i < interpolation_degree + 1; ++i) {
                ...
                for (int j = 0; j < interpolation_degree + 1; ++j) {
                    ...
                    for (int k = 0; k < interpolation_degree + 1; ++k) {
                        ...
                        T wijkdxj = dwijdxj * wk;
                        T wijkdxk = wij * one_over_dx * dw[2](k);
                        target(coord, wijk, Vector<T, dim>{ wijkdxi, wijkdxj, wijkdxk });
                    }
                }
            }
        }
    ...
    template <typename OP>
    inline void iterateKernel(const BSplineWeights<T, dim, degree>& spline, uint64_t base_offset, const OP& target)
    {
        auto grid_array = grid->Get_Array();
        T one_over_dx = spline.one_over_dx;
        auto& w = spline.w;
    ...
    template <typename OP>
    inline void iterateKernelWithDDW(const BSplineWeightsWithDDW<T, dim>& spline, uint64_t base_offset, const OP& target)
    {
        auto grid_array = grid->Get_Array();
        T one_over_dx = spline.one_over_dx;
        auto& w = spline.w;
        ...
        if constexpr (dim == 2) {
            for (int i = 0; i < interpolation_degree + 1; ++i) {
                ...
                for (int j = 0; j < interpolation_degree + 1; ++j) {
                    ...
                    target(coord, wij, Vector<T, dim>{ dwijdxi, dwijdxj }, ddw, g);
                }
            }
        }
        else {
    ...
    template <typename OP>
    inline void iterateKernelWithLaplacian(const BSplineWeightsWithDDW<T, dim>& spline, uint64_t base_offset, const OP& target)
    {
        auto grid_array = grid->Get_Array();
        T one_over_dx = spline.one_over_dx;
        auto& w = spline.w;
        ...
        //ZIRAN_ASSERT(dim == 2, "iterateKernelWithLaplacians only implemented for dim = 2");
        ...
        if constexpr (dim == 2) {
            for (int i = 0; i < interpolation_degree + 1; ++i) {
                ...
                for (int j = 0; j < interpolation_degree + 1; ++j) {
                    ...
                    target(coord, wij, Vector<T, dim>{ dwijdxi, dwijdxj }, laplacian, g);
                }
            }
        }
        else {
            for (int i = 0; i < interpolation_degree + 1; ++i) {
                ...
                for (int j = 0; j < interpolation_degree + 1; ++j) {
                    ...
                    for (int k = 0; k < interpolation_degree + 1; ++k) {
                        ...
                        target(coord, wijk, Vector<T, dim>{ wijkdxi, wijkdxj, wijkdxk }, laplacian, g);
                    }
                }
            }
        }
    ...
    template <typename OP>
    inline void iterateKernelP2G2P(const BSplineWeights<T, dim>& spline, uint64_t base_offset, const OP& target)
    {
        auto grid_array = grid->Get_Array();
        T one_over_dx = spline.one_over_dx;
        ...
        if constexpr (dim == 2) {
            for (int i = -interpolation_degree; i < 2 * interpolation_degree + 1; ++i) {

                coord[0] = base_coord[0] + i;
                for (int j = -interpolation_degree; j < 2 * interpolation_degree + 1; ++j) {
                    coord[1] = base_coord[1] + j;
                    auto offset = SparseMask::Packed_Add(base_offset, SparseMask::Linear_Offset(i, j));
                    GridState<T, dim>& g = reinterpret_cast<GridState<T, dim>&>(grid_array(offset));
                    target(coord, g);
            ...
            for (int i = -interpolation_degree; i < 2 * interpolation_degree + 1; ++i) {
                ...
                for (int j = -interpolation_degree; j < 2 * interpolation_degree + 1; ++j) {
                    coord[1] = base_coord[1] + j;
                    for (int k = -interpolation_degree; k < 2 * interpolation_degree + 1; ++k) {
                        coord[2] = base_coord[2] + k;
                        auto offset = SparseMask::Packed_Add(base_offset, SparseMask::Linear_Offset(i, j, k));
                        GridState<T, dim>& g = reinterpret_cast<GridState<T, dim>&>(grid_array(offset));
                        target(coord, g);
