...
bool implicit = Python::implicit;

sim.end_frame = 192; //72 for p and FS, 216 for eta, and 192 for youngs!!
T frameRate = 48; //24 for percent, FS, and youngs, 72fps for eta!!
...
if (Python::isotropic) {
    std::string path("output/3D_TestImplicit/3D_TestImplicit_Isotropic_Youngs" + E_str + "_percent" + percent_str + "_eta" + eta_str + "_dx" + dx_str + "_fiberScale" + scale_str + "_endTime");
    sim.output_dir.path = path;
}
else if (implicit) {
    std::string path("output/3D_TestImplicit/3D_TestImplicit_Implicit_Youngs" + E_str + "_percent" + percent_str + "_eta" + eta_str + "_dx" + dx_str + "_fiberScale" + scale_str + "_residual" + res_str + "_endTime");
    sim.output_dir.path = path;
    model.setExtraFiberStiffness(0, Python::fiberScale); //only scale elasticity if anisotropic!
}
else {
    std::string path("output/3D_TestImplicit/3D_TestImplicit_Explicit_Youngs" + E_str + "_percent" + percent_str + "_eta" + eta_str + "_dx" + dx_str + "_fiberScale" + scale_str + "_residual" + res_str + "_endTime");
    sim.output_dir.path = path;
    model.setExtraFiberStiffness(0, Python::fiberScale); //only scale elasticity if anisotropic!
}
